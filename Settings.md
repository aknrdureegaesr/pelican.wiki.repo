...
--------------|-------------
`{attach}` | directory path to where this content is located in. Useful for image or A links.
`{author}` |
`{category}` | Returns the current category found in content file's metadata.
`{date}` | Returns the date found in content file's metadata
`{filename}` | 
`{index}` | returns index URL that this content file is filed under.  See `$INDEX_SAVE_AS` setting.
`{lang}` |  Two-character country code; defaults to 'en'.
`{path}` |
`{save_as}` | obtains the `_SAVE_AS` settings based on the type of content.
`{static}` |
`{slug}` | Returns the slug found in content file's metadatai, otherwise computes a slug name derived from title metadata fund in  content file, otherwise use own filename of its content
`{summary}` | Returns the summary found in content file's metadata.
`{tag}` |
`{url}` | the finalized computed URL of this content file.

Pelican Generic Settings
==========
...
==============================
The most complete list of Pelican's Python-based configuration setting names are:
